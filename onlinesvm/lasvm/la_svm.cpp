...
vector <double> cov_pos;
vector <double> cov_neg;
    ...
    if(kernel_type==RBF)
    {
        ...
        for(i=0;i<msz;i++){
         if(Y[i+m] == 1){
            ...
            x_square[i+m]=x_square[i+m]+cov_pos[i];
         }
         else{
            x_square[i+m]=lasvm_sparsevector_dot_product(X[i+m],X[i+m]); 
            x_square[i+m]=x_square[i+m]+cov_neg[i];
         }
        }
    }
...
int main(int argc, char **argv)  
{
    ...
    cov_pos.push_back(-0.0405);
    cov_pos.push_back(-0.3223);
    cov_pos.push_back(0.1240);
    cov_pos.push_back(0.3398);
    cov_pos.push_back(-0.0742);
    cov_pos.push_back(-0.1699);
    cov_pos.push_back(-0.0894);
    cov_pos.push_back(0.0659);

    cov_neg.push_back(-0.0918);
    cov_neg.push_back(0.1523);
    cov_neg.push_back(0.0552);
    cov_neg.push_back(-0.3223);
    cov_neg.push_back(-0.0908);
    cov_neg.push_back(-0.3320);
    cov_neg.push_back(0.1113);
    cov_neg.push_back(0.0510);
